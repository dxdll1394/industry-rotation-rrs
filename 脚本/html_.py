...
function buildAnalysisReport() {{
  ...
  html += '<div style=\"display:flex;align-items:baseline;gap:8px;margin-bottom:6px\"><span style=\"font-size:16px;font-weight:bold\">行业RRS分析</span><span style=\"font-size:10px;color:#888\">' + dateStr + '</span></div>';

  // Summary mini-bar
  html += '<div style=\"display:flex;gap:6px;margin-bottom:12px;font-size:10px\">';
  ['L','I','W','G'].forEach(function(q) {{
    var clr = {{L:'#1565C0',I:'#2E7D32',W:'#E65100',G:'#999'}};
    html += '<span style=\"background:' + clr[q] + ';color:#fff;padding:2px 8px;border-radius:3px\">' + q + ':' + counts[q] + '</span>';
  }});
  html += '<span style=\"color:#888;margin-left:4px\">L+I ' + liPct + '% (共' + total + '个)</span>';
  html += '</div>';

  // ===== WAVE ANALYSIS (primary) =====
  html += '<h3 style=\"font-size:13px;margin:16px 0 8px;color:#333\">🌊 波浪位置</h3>';
  ...
  var phaseBg = {{main_advance:'#E8F5E9', strong_advance:'#E8F5E9', pre_main:'#FFF8E1', terminal:'#FFEBEE', post_main:'#f8f8f8'}};
  ...
  wkeys.forEach(function(sec) {{
    ...
    html += '<div style=\"background:' + (phaseBg[w.phase]||'#f8f8f8') + ';border-radius:6px;padding:6px 10px;min-width:140px;flex:1;max-width:220px\">';
    html += '<div style=\"font-weight:bold;font-size:12px\">' + icon + ' ' + sec + '</div>';
    html += '<div style=\"font-size:10px;color:#666\">RS ' + w.latest_rs.toFixed(0) + ' | ' + phaseLabels[w.phase] + '</div>';
    html += '<div style=\"font-size:9px;color:#888\">' + dir + w.current_amp.toFixed(0) + '/' + w.current_days + '天 (主升' + w.main_amp.toFixed(0) + '/' + w.main_days + '天)</div>';
    html += '</div>';
  }});
  html += '</div>';

  // ===== QUADRANTS (collapsible) =====
  html += '<h3 style=\"font-size:13px;margin:16px 0 8px;cursor:pointer;color:#333\" onclick=\"var el=document.getElementById(\\'quadrantSec\\');el.style.display=el.style.display===\\'none\\'?\\'block\\':\\'none\\'\">📊 象限明细 ▾</h3>';
  html += '<div id=\"quadrantSec\" style=\"display:none\">';
  var qNames = {{L:'强势',I:'改善',W:'走弱',G:'低迷'}};
  var qClr = {{L:'#1565C0',I:'#2E7D32',W:'#E65100',G:'#999'}};
  ['L','I','W','G'].forEach(function(q) {{
    var items = sorted.filter(function(s) {{ return (snap ? getSnapQuad(s, snap) : s.quad) === q; }});
    if (!items.length) return;
    html += '<div style=\"margin:4px 0 8px\"><span style=\"font-weight:bold;color:' + qClr[q] + ';font-size:11px\">' + qNames[q] + ' (' + items.length + ')</span>';
    html += '<span style=\"font-size:10px;color:#888\"> ';
    items.forEach(function(s, i) {{
      ...
      html += wi + s.name + '(' + latest[0].toFixed(0) + '/' + (latest[1]>=0?'+':'') + latest[1].toFixed(0) + sign + Math.abs(cd) + ')';
  ...
  // ===== WINDOW + ALERTS =====
  html += '<h3 style=\"font-size:13px;margin:16px 0 8px;color:#333\">🎯 机会 & 警报</h3>';
  html += '<div style=\"display:flex;gap:12px;flex-wrap:wrap\">';

  // Left: window candidates
  html += '<div style=\"flex:1;min-width:200px\">';
  html += '<div style=\"font-size:11px;font-weight:bold;color:#1565C0;margin-bottom:4px\">逆转窗口</div>';
  var found = false;
  sorted.forEach(function(s) {{
    ...
    found = true;
    var star = downDays >= 15 && downDays <= 24 ? '★' : '';
    html += '<div style=\"font-size:10px;padding:2px 0' + (star?';background:#FFF8E1;border-radius:3px;padding:2px 4px':'') + '\">' + star + ' ' + s.name + ' ↓' + downDays + '日 RS' + latest[0].toFixed(0) + '</div>';
  }});
  if (!found) html += '<div style=\"font-size:10px;color:#888\">无</div>';
  html += '</div>';
  ...
  html += '<div style=\"flex:1;min-width:200px\">';
  html += '<div style=\"font-size:11px;font-weight:bold;color:#d32f2f;margin-bottom:4px\">警报区</div>';
  if (danger.length) {{
    danger.forEach(function(s) {{
      var latest = getSectorLatest(s, snap);
      var traj = getTrajUpto(s, snap);
      var cd = consecDir(traj.map(function(p){{return p.value;}}).reverse());
      html += '<div style=\"font-size:10px;color:#c62828;padding:2px 0\">⚠ ' + s.name + ' ↓' + (Math.abs(cd)*5) + '日</div>';
    }});
  }} else {{
    html += '<div style=\"font-size:10px;color:#888\">无</div>';
  }}
  html += '</div>';
  html += '</div>';

  html += '<div style=\"margin-top:12px;font-size:9px;color:#bbb;text-align:center\">行业轮动 RRS · 自动生成</div>';
  html += '</div>';
